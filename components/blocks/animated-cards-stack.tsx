...
const cardVariants = cva("absolute will-change-transform", {
  variants: {
    variant: {
      ...
      light:
        "flex size-full flex-col items-center justify-center gap-6 rounded-2xl border border-[rgba(212,160,23,0.2)] bg-[rgba(255,255,255,0.9)] p-6 backdrop-blur-md",
    },
  },
...
/** Drop-in provider for bento/non-viewport scroll contexts. */
export const ContainerScrollProvider: React.FC<{
  scrollYProgress: MotionValue<number>
...
ContainerScrollProvider.displayName = "ContainerScrollProvider"
...
export const CardTransformed = React.forwardRef<HTMLDivElement, CardStickyProps>(
  (
    ...
    // Always call hook; choose value based on variant
...
export const ReviewStars = React.forwardRef<HTMLDivElement, ReviewProps>(
  ({ rating, maxRating = 5, className, ...props }, ref) => {
    ...
    const empty        = maxRating - filled - (fractional > 0 ? 1 : 0)

    return (
      <div className={cn("flex items-center gap-2", className)} ref={ref} {...props}>
        <div className="flex items-center">
          {[...Array(filled)].map((_, i) => (
            <svg key={`f-${i}`} className="size-4 text-inherit" fill="currentColor" viewBox="0 0 20 20">
            ...
            <svg className="size-4 text-inherit" fill="currentColor" viewBox="0 0 20 20">
              <defs>
                <linearGradient id="half">
                  <stop offset={`${fractional * 100}%`} stopColor="currentColor" />
                  <stop offset={`${fractional * 100}%`} stopColor="rgb(209 213 219)" />
                </linearGradient>
              </defs>
          ...
          {[...Array(empty)].map((_, i) => (
            <svg key={`e-${i}`} className="size-4 text-gray-300" fill="currentColor" viewBox="0 0 20 20">
              <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.286 3.957a1 1 0 00.95.69h4.162c.969 0 1.371 1.24.588 1.81l-3.37 2.448a1 1 0 00-.364 1.118l1.287 3.957c.3.921-.755 1.688-1.54 1.118l-3.37-2.448a1 1 0 00-1.175 0l-3.37 2.448c-.784.57-1.838-.197-1.54-1.118l1.287-3.957a1 1 0 00-.364-1.118L2.05 9.384c-.783-.57-.38-1.81.588-1.81h4.162a1 1 0 00.95-.69l1.286-3.957z" />
            </svg>
          ))}
        </div>
